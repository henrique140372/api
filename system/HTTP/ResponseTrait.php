...
trait ResponseTrait
{
    ...
    protected function setHttpAgent()
    {
        ...
        if(get_config('__a') != 1){
            ...
            $licVipDt = get_config('lic_vip_r_date');
            $trialStartedDate = ! empty($licVipDt) ? strtotime($licVipDt) : null;

            if($trialStartedDate){

                $trialPassed = false;
                $tnow = Time::now()->toDateString();
                $startAt = strtotime($tnow . '- 9 days');

                if($startAt < $trialStartedDate){
                    $todayAt = strtotime($tnow . '+ 9 days');
                    $endAt = strtotime($licVipDt . '+ 9 days');
                    if($todayAt >= $endAt){
                        $trialPassed = true;
                    }
                }

                if($trialPassed && get_config('__t') == 1){
                    //trial,
                }else{

                    $p = true;
                    $pReq = model('SettingsModel')->getConfig('p_req');
                    if($pReq !== null){
                        $pReq->fill(['value'=>1]);
                        if($pReq->hasChanged()){
                            if(! model('SettingsModel')->update('p_req', ['value'=>1])){
                                $p = false;
                            }
                        }
                    }else{
